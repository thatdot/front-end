...
import com.quine.language.ast.{Effect, Expression, Operator, Predicate, Projection, Query, Source, Value}
...
object SQLParser {
  ...
  val ident: Parser[Expression.Ident] = alpha.repAs[String].map(s => Expression.Ident(Source.NoSource, Symbol(s)))
  ...
  val fieldAccessExpression: Parser[Expression] = Parser.recursive[Expression] { recurse =>
    ...
    // Combine the parsers
    (ident ~ dotExpr.?).map {
      case (prefix, Some(suffix)) => Expression.BinOp(Source.NoSource, Operator.Dot, prefix, suffix)
  ...
  val stringLit: Parser[Expression] = strchr.rep.string.between(sq, sq).map(v => Expression.Literal(Source.NoSource, Value.Text(v)))
  val intLit: Parser[Expression] = digit.rep.string.map(v => Expression.Literal(Source.NoSource, Value.Integral(v.toInt)))
  ...
  val exp = Parser.recursive[Expression] { r =>
    ...
    val binOp: Parser[Expression] = (lhs $ (op $ r)).map(p => Expression.BinOp(Source.NoSource, p._2._1, p._1, p._2._2)).backtrack
  ...
  def expToString(exp: Expression): String = exp match {
    case Expression.Literal(src, value) => value match {
    ...
    case Expression.Ident(src, name) => name.name
    case Expression.Parameter(src, name) => name.name
    case Expression.Apply(src, name, args) => s"$name(${args.mkString(",")})"
    case Expression.UnaryOp(src, op, exp) => s"${opToString(op)}${expToString(exp)}"
    case Expression.BinOp(src, op, lhs, rhs) => s"(${expToString(lhs)} ${opToString(op)} ${expToString(rhs)})"
  }
  ...
  val aliasedTable = (labels ~ ident).map { p =>
    Predicate.ExistsNode(Source.NoSource, p._2.name, p._1.toList.map(s => Symbol(s)))
  }
  val table = label.map { l =>
    Predicate.ExistsNode(Source.NoSource, CheatingNameGenerator.synthesizeIdent(), List(Symbol(l)))
  }
  ...
  val tablePart = (tableExp ~ joinPart.surroundedBy(whitespaces0).rep0).map { p =>
    ...
    p._2.foldLeft[(Predicate, Symbol)](p._1 -> initName) { (pred, token) =>
      ...
      val edgeExists = Predicate.ExistsEdge(Source.NoSource, CheatingNameGenerator.synthesizeIdent(), token._2._2._2.toList.map(name => Symbol(name)), sourceName, destName)
      val newPred = Predicate.And(Source.NoSource, pred._1, edgeExists)
      newPred -> destName
    }._1
  }
  ...
  val whereClause = Parser.recursive[Predicate] { r =>
    ...
    val inner = exp.map(e => Predicate.Satisfies(Source.NoSource, e))
    val orExps: Parser[Predicate] = (inner $ (or #> r)).map(p => Predicate.Or(Source.NoSource, p._1, p._2)).backtrack
    val andExps = (inner $ (and #> r)).map(p => Predicate.And(Source.NoSource, p._1, p._2)).backtrack
  ...
  val query =
    (selectBit.surroundedBy(whitespaces0) ~
      ...
      Query.Single(
        ...
        predicate = Predicate.And(Source.NoSource, p._1._2.toList.foldLeft(Predicate.sat)((prev,curr) => Predicate.And(Source.NoSource, prev, curr)), p._2.getOrElse(Predicate.True)),
  ...
  private def rewriteQuery(q: Query, r: Rewriter): Query = q match {
    case Query.Union(src, lhs, rhs) => Query.Union(src, rewriteQuery(lhs, r), rewriteQuery(rhs, r))

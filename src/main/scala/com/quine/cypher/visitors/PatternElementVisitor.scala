...
object PatternElementVisitor extends CypherBaseVisitor[Predicate] {
  override def visitOC_PatternElement(ctx: OC_PatternElementContext): Predicate = {
    ...
    val leftName = r1 match {
      case Some(Predicate.ExistsNode(src, lhs, _)) => lhs
      case _ => throw new RuntimeException("This shouldn't happen!")
    }

    val r3 = maybeMatchList(ctx.oC_PatternElementChain(), new PatternElementChainVisitor(leftName)).map(chain => chain.foldRight(Predicate.sat)((chainPred, pred) => Predicate.And(Source.NoSource, pred, chainPred)))

    val lhs = (r1 <+> r2).get
    val rhs = r3.getOrElse(Predicate.sat)

    Predicate.And(src, lhs, rhs)
  }
}
